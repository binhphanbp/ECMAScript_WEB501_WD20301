...
function getActiveVoucher() {
  // Lấy voucher từ UI (chỉ dùng cho trang cart.html)
  return (
...
function getSelectedVoucherCodeFromCart() {
  // Lấy voucher đã lưu trong sessionStorage khi chuyển từ Cart sang Checkout
...
// isCartPage: TRUE nếu đang tính tổng cho trang cart (KHÔNG tính phí ship vào finalTotal)
function calculateCartTotals(cart, currentVoucher, isCartPage = false) {
  ...
  // Ở trang Cart, Tổng thanh toán KHÔNG bao gồm phí ship, chỉ bằng (Tạm tính - Giảm giá)
  const totalBeforeShipping = subtotal - discount;
  const finalTotal = isCartPage
    ? totalBeforeShipping
    : totalBeforeShipping + shippingFee;
...
function renderCart() {
  ...
  // --- Render VOUCHER ---
  // Sử dụng voucher đã lưu trong session để hiển thị trạng thái active
...
function selectVoucher(element) {
  ...
  // Lưu voucher đã chọn vào sessionStorage để đồng bộ với trang checkout
...
function updateCartTotals() {
  ...
  const cart = getCart();
  // Gửi tham số TRUE: tính toán cho trang Giỏ hàng (Total = Subtotal - Discount)
  const totals = calculateCartTotals(cart, voucher, true);
  ...
  if (shippingFeeCartEl)
    shippingFeeCartEl.textContent = formatCurrency(totals.shippingFee);
  if (grandTotalEl)
...
function renderCheckoutSummary() {
  // Lấy voucher đã chọn từ trang cart (hoặc mặc định nếu không chọn)
  const activeVoucherCode = getSelectedVoucherCodeFromCart();

  const cart = getCart();
  // Gửi tham số FALSE: tính toán tổng cuối cùng (có phí ship)
  const totals = calculateCartTotals(cart, activeVoucherCode, false);
...
function placeOrder() {
  ...
  // Tính toán lại tổng cuối cùng trước khi lưu
  const orderTotals = calculateCartTotals(cart, activeVoucherCode, false);
  ...
  // 2. Xóa giỏ hàng và voucher session
  saveCart([]);
