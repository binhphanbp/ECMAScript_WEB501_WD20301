...
class Player extends Human {
  constructor(
    ...
    this.goals = goals;
    this.isGoldenBall = isGoldenBall; // 🏅 boolean
  }

  goldenBall = () => `${this.name} has won the Golden Ball! 🏅`;
...
// ===============================
// 🎯 CÁC YÊU CẦU THEO BÀI
// ===============================

// 1️⃣ Hiển thị thông tin 1 cầu thủ cụ thể
const showPlayerInfo = (name) => {
  const player = players.find((p) => p.name === name);
  console.log(player ? player.getInfo() : '⚠️ Player not found!');
};

// 2️⃣ Lấy chi tiết: quê quán & tuổi
const getPlayerDetails = (name) => {
  const player = players.find((p) => p.name === name);
  if (player) {
    console.log(
      `🏡 Address: ${player.getAddress()}, 🎂 Age: ${player.getAge()}`
    );
  } else {
    console.log('⚠️ Player not found!');
  }
