//? ------------- FOREACH() -------------
//* forEach là phương thức duyệt qua từng phần tử trong mảng
const numberList = [3, 2, 4, 7, 9];
numberList.forEach((number) => console.log(number * 2));

//? In ra bình phương các số
numberList.forEach((number) => console.log(number * number));

//? In ra các số chẵn
numberList.forEach((number) => {
  if (number % 2 === 0) {
    console.log(number);
  }
});

//? In ra các số lớn hơn 5
numberList.forEach((number) => {
  if (number > 5) {
    console.log(number);
  }
});

const users = [
  {
    name: 'Cường',
    age: 21,
  },
  {
    name: 'Nam',
    age: 25,
  },
  {
    name: 'Bình',
    age: 22,
  },
];
users.forEach((user) => console.log(`${user.name} có tuổi là ${user.age}`));

//? ------------- FILTER() -------------
//* filter là phương thức lọc dữ liệu trong mảng và trả về mảng mới thoả điều kiện
//? Lọc ra mảng số chẵn dựa vào mảng numberList
const newNumbers = numberList.filter((number) => number % 2 === 0);
console.log(newNumbers); // [4, 8]

//? Dùng filter lấy mảng user có tuổi lớn hơn 8 dựa vào mảng users
const newUsers = users.filter((user) => user.age > 18);
console.log(newUsers); // [ { name: 'Cường', age: 21 }, { name: 'Name', age: 25 } ]

//? ------------- MAP() -------------
//* map là phương thức lớp dữ liệu trong mảng và trả về mảng mới
//? Tạo mảng bình phương các số dựa vào mảng numberList
const doubleArr = numberList.map((number) => number * 2);
console.log(doubleArr); // [6, 4, 8, 14, 18]

//? Tạo mảng chứa tên các user trong object users
const userNameOfUsersArray = users.map((user) => user.name);
console.log(userNameOfUsersArray); // [ 'Cường', 'Nam', 'Bình' ]

//? Trả về mảng fruits với các chữ viết hoa
const fruits = ['orange', 'mango', 'watermelon'];
const upperFruits = fruits.map((fruit) => fruit.toUpperCase());
console.log('uppercaseFruits:', upperFruits); // upperFruits: [ 'ORANGE', 'MANGO', 'WATERMELON' ]

//? ------------- REDUCE() -------------
//* reduce là phương thức lớp dữ liệu trong mảng và trả về 1 giá trị
//? Tính tổng các số trong mảng numberList
const total = numberList.reduce((tempTotal, number) => tempTotal + number, 0);
console.log(total); // 25
