...
const SAVED_ADDRESSES = [
    { name: "Nguyễn Văn B", phone: "0901234567", address: "123 Đường A, Phường X, Quận B, TP.HCM" },
    { name: "Trần Thị C", phone: "0911223344", address: "12 Ký Con, Phường Nguyễn Thái Bình, Quận 1, TP.HCM" },
];
...
function getInitialCart() {
    return [
        { id: 'SP1', name: "Áo Thun Polo Classic", options: "Màu: Đen, Size: L", price: 250000, quantity: 1, img: 'SP1' },
        { id: 'SP2', name: "Sách 'Kỹ Thuật Sống'", options: "Tác giả: Adam Khoo", price: 120000, quantity: 2, img: 'SP2' },
        { id: 'SP3', name: "Nước hoa X", options: "Size: 50ml", price: 800000, quantity: 1, img: 'SP3' },
    ];
}
...
function getActiveVoucher() {
    return document.querySelector('.voucher-item.active')?.dataset.code || 'SALE10';
}
        ...
        const html = `
            <div class="cart-item" data-id="${item.id}">
                ...
                <div class="item-actions">
                    ...
                    <span class="item-subtotal" id="subtotal-${item.id}">${(item.price * item.quantity).toLocaleString('vi-VN')}₫</span>
...
function updateQuantity(itemId, delta) {
    ...
    if (itemIndex > -1) {
        ...
        document.getElementById(`subtotal-${itemId}`).textContent = newSubtotal.toLocaleString('vi-VN') + '₫';
...
function updateCartTotals() {
    ...
    if (subtotalEl) subtotalEl.textContent = totals.subtotal.toLocaleString('vi-VN') + '₫';
    if (itemCountEl) itemCountEl.textContent = totals.itemCount;
    // Tổng thanh toán bao gồm phí ship và giảm giá
    if (grandTotalEl) grandTotalEl.textContent = totals.finalTotal.toLocaleString('vi-VN') + '₫';
}
...
function renderCheckoutSummary() {
    ...
    if (summaryItemsEl) {
        summaryItemsEl.innerHTML = cart.map(item => `
            <div class="order-item-summary">
                <span>${item.name} (${item.options.replace(/, /g, ', ')}, x${item.quantity})</span>
    ...
    // Cập nhật tổng kết
    document.getElementById('checkout-subtotal').textContent = totals.subtotal.toLocaleString('vi-VN') + '₫';
    document.getElementById('shipping-fee').textContent = totals.shippingFee.toLocaleString('vi-VN') + '₫';
    document.getElementById('checkout-discount').textContent = '-' + totals.discount.toLocaleString('vi-VN') + '₫';
    document.getElementById('discount-code-label').textContent = totals.discountLabel;
    document.getElementById('checkout-grand-total').textContent = totals.finalTotal.toLocaleString('vi-VN') + '₫';
...
function placeOrder() {
    ...
    modalContent.innerHTML = `
        <div class="order-summary-box">
            <p><strong>Mã đơn hàng:</strong> <span>${orderDetails.orderId}</span></p>
            <p><strong>Ngày đặt hàng:</strong> <span>${orderDetails.orderDate}</span></p>
        </div>
        
        <h3>🚚 Thông tin giao hàng</h3>
        <div class="info-group">
            <p><strong>Tên người nhận:</strong> ${orderDetails.shippingInfo.fullName}</p>
            <p><strong>Điện thoại:</strong> ${orderDetails.shippingInfo.phone}</p>
            <p><strong>Địa chỉ:</strong> ${orderDetails.shippingInfo.address}</p>
            <p><strong>Ghi chú:</strong> ${orderDetails.shippingInfo.note || 'Không có'}</p>
        </div>
