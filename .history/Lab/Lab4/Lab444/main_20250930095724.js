...
const DEFAULT_SHIPPING_FEE = 20000; // Phí vận chuyển mặc định 20.000₫ (Dùng cho tính toán)
...
// --- Hàm chung: Local Storage (Đã Cập Nhật để đảm bảo dữ liệu mẫu) ---
function getCart() {
  const cart = localStorage.getItem(LOCAL_STORAGE_KEY);
  if (!cart || JSON.parse(cart).length === 0) {
    // Nếu Local Storage trống hoặc mảng rỗng, tạo và trả về giỏ hàng mẫu
...
// --- Cart Page Logic ---
function renderCart() {
  // Luôn lấy cart, nếu trống thì sẽ load dữ liệu mẫu
  const cart = getCart();
  ...
  if (cart.length === 0) {
    listElement.innerHTML = `<p style="text-align: center; color: #7f8c8d; padding: 30px;">Giỏ hàng của bạn hiện đang trống. Vui lòng mua sắm thêm!</p>`;
  } else {
    cart.forEach((item) => {
      const html = `
...
function renderAddressSelector() {
  ...
  // Thêm các địa chỉ đã lưu (DỮ LIỆU MẪU)
...
function loadSelectedAddress() {
  ...
  if (index === 'new-address') {
    // Xóa sạch các trường khi chọn Thêm địa chỉ mới
    fullNameEl.value = '';
    phoneEl.value = '';
  ...
  // Load dữ liệu mẫu nếu chọn địa chỉ đã lưu
...
function renderCheckoutSummary() {
  // Giả định voucher đang áp dụng là SALE10 (hoặc có thể tùy chỉnh)
  const activeVoucherCode = 'SALE10';

  // Lấy giỏ hàng (đã đảm bảo có dữ liệu mẫu)
  const cart = getCart();
